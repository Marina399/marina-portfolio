...
const modules={
  'report-builder':{
    ...
    outcome:'No more Excel merging. Admins build their own reports in minutes. Developer requests for custom reports dropped to zero.',
    outcomeQuote:'"No more Excel merging. Admins build their own reports in minutes."',
    uat:'Tested with 5 nursery admins across different roles (finance, management, operations). Handoff included component specifications, filter logic decision tree, responsive behavior on tablet and desktop, and RTL support for Arabic.',
  ...
  'schedule-plan':{
    ...
    tagStyle:'tag-orange',
    problem:'Teachers monthly created a full plan for each child — 20 children meant 20 separate plans. Manual entry for each payer, discount, and subsidy per child. No flexibility for split payments or edge cases. No scalable solution.',
    ...
    decisions:[
      ...
      {num:'02',title:'Regional flexibility',body:'Canada splits by session or item (up to 5 payers). Middle East splits by percentage. Same UI components, different logic underneath.'},
      {num:'03',title:'Override without breaking',body:'Editing one child\'s plan never affects the rest of the bulk. Group logic and individual logic are completely separated.'}
    ],
    flow:[
      {label:'Setup & Build Your Plan',desc:'Define dates, pricing group, sessions, items, taxes.',img:'./src/images/SP-step1.png'},
      {label:'Assign Children & Payers',desc:'Bulk assign to a room, then customize exceptions.',img:'./src/images/SP-step2.png'},
      {label:'Customize Payers',desc:'Edit individual payer splits per child.',img:'./src/images/SP-step3.png'},
    ...
    outcome:'500 children\'s plans created in under 10 minutes. Teachers save 15+ hours per week.',
    outcomeQuote:'"From hours to minutes. Teachers finally have time to teach."',
    uat:'Tested with teachers and finance admins from Canada and the Middle East. Handoff included flow diagrams for each edge case and responsive views for tablet.',
  ...
  'child-development':{
    ...
    decisions:[
      ...
      {num:'02',title:'Cascading filters',body:'Choosing a curriculum automatically shows only its areas and subareas. Users never see invalid combinations.'},
      {num:'03',title:'RTL mirroring',body:'Full layout reversal for Arabic: tables, buttons, icons, and navigation all flipped correctly.'}
    ],
    flow:[
    ...
    outcome:'Teachers see at-risk children in 2 seconds without clicking a single filter.',
    outcomeQuote:'"Teachers see at-risk children in 2 seconds, without clicking a single filter."',
    uat:'Tested with teachers on actual devices (iPad, iPhone, Android tablets, desktops). Handoff included responsive breakpoints and RTL CSS specifications.',
  ...
  'invoice-automations':{
    ...
    flow:[
      ...
      {label:'Set reminder schedule',desc:'Define when to send and to whom.',img:'./src/images/Invoice Automations (4).png'},
    ...
    outcome:'Admins save 20+ hours per month. The only module that completed the full product cycle with zero critical bugs.',
    outcomeQuote:'"The only module that went from spec to live with zero critical bugs."',
    uat:'Tested with finance admins across Canada and the Middle East. Handoff included edge cases (pro-rata, mid-cycle cancellations, multi-payer splits) and email template specs.',
    prev:'child-development',next:'waitlist'
  },
  'waitlist':{
    title:'Waitlist Management',
    subtitle:'A flexible, fair admissions pipeline with custom priorities, bulk actions, and personalised table views.',
    role:'Senior Product Designer – End-to-end ownership',
    duration:'',
    heroImg:'./src/images/WaitList.png',
    tags:['Auto-enrollment','Priority management','Real-time updates','Nursery admissions'],
    tagStyle:'tag-orange',
    problem:'Waitlists were messy and unfair. Nurseries used first-come, first-served FIFO ordering — a child with an enrolled sibling or urgent need had no way to jump the queue. Manual work multiplied when dozens of children needed the same priority assignment.',
    problemImg:'./src/images/WaitList (2).png',
    question:'"How do you turn a basic waitlist into a transparent, rule-based system where every nursery can define its own priorities?"',
    ...
    decisions:[
      {num:'01',title:'Priority Management System',body:'Admins create named priorities (e.g. "Sibling Enrolled"), assign a distinct colour, and reorder them via drag & drop.'},
      {num:'02',title:'Customisable Columns',body:'Every user can choose which columns to display and drag columns to reorder them. The layout persists per user.'},
      {num:'03',title:'Bulk Actions',body:'Staff select multiple inquiries and assign a priority in one click. Bulk actions also support status changes and mass messaging.'}
    ],
    flow:[
      {label:'Apply & join waitlist',desc:'Parent applies online, automatically added with timestamp.',img:'./src/images/WaitList (3).png'},
      {label:'Spot opens: 48h window',desc:'System notifies the next family. 48 hours to accept.',img:'./src/images/WaitList (4).png'},
    ...
    outcome:'Admissions teams save 15+ hours per week. Nurseries fill open spots 3x faster than before.',
    outcomeQuote:'"From a rigid list to a flexible, fair system – nurseries finally control their own admission rules."',
    uat:'Tested with nursery admissions staff across different roles. Handoff included priority calculation logic, drag-drop behaviour, and bulk action edge cases.',
  ...
  'global-search':{
    ...
    tagStyle:'tag-orange',
    problem:'7+ entities (Children, Payers, Staff, Billing, Rooms, Inquiries, Documents), each with its own search. Users had to know exactly where to look. No shortcuts for power users.',
    problemImg:'./src/images/Global Search (2).png',
    question:'"How do you let users find anything from one place, in seconds?"',
    ...
    decisions:[
      ...
      {num:'02',title:'Tabbed results',body:'Results grouped by entity type. Users can filter by tab without retyping the query.'},
      {num:'03',title:'Location-aware',body:'Search includes current nursery location. One-click to switch locations — critical for multi-site organizations.'}
    ],
    flow:[
      {label:'Type anywhere',desc:'Autocomplete shows top 3 matches per entity as you type.',img:'./src/images/Global Search (3).png'},
      {label:'Results grouped by entity',desc:'Hit Enter to see full results with previews.',img:'./src/images/Global Search (4).png'},
    ...
    outcome:'Search time reduced by 70%. Admins save 10+ hours per week.',
    outcomeQuote:'"Search is finally fast. No more guessing which module to open."',
    uat:'Tested with nursery admins, finance staff, and teachers. Handoff included debouncing logic, result ranking algorithm documentation, and a keyboard shortcut guide.',
...
function showModule(slug){
  ...
  if(!d) return;
  const prevBtn=`<button class="case-nav-btn" data-nav="${d.prev}">← ${moduleTitles[d.prev]}</button>`;
  const nextBtn=`<button class="case-nav-btn" data-nav="${d.next}">${moduleTitles[d.next]} →</button>`;
  const tagsHTML=d.tags.map(t=>`<span class="tag ${d.tagStyle}">${t}</span>`).join('');
  const roleHTML=d.myRole.map(r=>`<li>${r}</li>`).join('');
  const decsHTML=d.decisions.map((dc,i)=>`
    <div class="dec-card reveal d${i+1}">
  ...
  document.getElementById('module-content').innerHTML=`
    <div class="cs-hero" style="background:var(--surface)">
      <div class="cs-hero-inner">
        <div>
          ...
          <div class="cs-role">${d.role}${d.duration?' · '+d.duration:''}</div>
        </div>
        <div style="border-radius:var(--r-md);overflow:hidden">
          <img src="${d.heroImg}" style="width:100%;display:block" alt="${d.title}" onerror="this.style.display='none'">
        </div>
      </div>
    </div>
        ...
        <div class="two-col two-col-center">
          ...
          <div class="reveal d2">
            ...
            <div style="border-radius:var(--r-lg);overflow:hidden;border:1px solid var(--border)">
              <img src="${d.problemImg||d.heroImg}" style="width:100%;display:block" alt="Challenge" onerror="this.style.display='none'">
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="section">
      <div class="container">
        ...
        <div class="case-nav">${prevBtn}${nextBtn}</div>
      </div>
    </div>`;
